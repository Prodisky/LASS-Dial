# LASS Dial
LASS Dial 依照地理位置與資料項目名稱查詢距離最近測站的最新資料，並加以呈現。
...
- 更新時間，例如：2016/01/23 14:31 +8

### 期望雲端資料查詢設計
依照地理位置與資料項目名稱查詢 JSON 格式資料，例如：
http://....../GetData/?lat=120.658836&lng=24.169699&data=O3&ttl=7200
- 不是所有的測站具備相同的資訊，好比說有的站沒有臭氧(O3)的資料，那上述的查詢，就需要傳回具有 O3 資料的測站中，距離指定坐標最近的測站資料。
- ttl為可選參數，可以進一步過濾需要多新的資料來參考，以秒數表示，上述的查詢，表示必須為兩小時內資料，以確保不會取得過期太久的資訊，可選原因在於，可能沒有資料，或符合要求測站的距離過遠，因此放寬時間參數。
